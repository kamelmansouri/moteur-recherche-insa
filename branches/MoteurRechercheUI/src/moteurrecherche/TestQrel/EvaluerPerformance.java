...
public class EvaluerPerformance {
    ...
    public EvaluerPerformance(File queries, int maxParagraphes, boolean withReasoner) throws ClassNotFoundException, SQLException,
...
//        File queries = new File(EvaluerPerformance.class.getResource("/resources/queries.xml").getPath());
        ...
        try {
            ...
            this.result = this.result.concat(title);
        ...
        for (int q = 1; q <= this.requetes.length; q++) {
            ...
            this.result = this.result.concat(text);
            writer.write(text);
        }
